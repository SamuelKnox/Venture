...
namespace Devdog.InventorySystem.Models
{
    ...
    public partial class InventoryCurrencyLookup
    {
        ...
        public delegate void CurrencyChanged(float before, InventoryCurrencyLookup after);
        ...
        public InventoryCurrencyLookup()
        {
            _currencyID = 0;
            _amount = 0;
        }
        ...
        private void TryAutoRefillCurrency()
        {
            ...
            if (_amount < 0f)
            {
                var conversionDict = new Dictionary<uint, float>();
                _GetConversionFactor(conversionDict, currency, 1f);
                foreach (var lookup in parentContainer.lookups)
                {
                    if (conversionDict.ContainsKey(lookup._currencyID))
                    {
                        // We can convert to this
                        // How much is it worth in another currency?
                        float amountInOtherCurrency = Mathf.Abs(_amount * conversionDict[lookup._currencyID]);
...
//                Debug.Log("Trying other method... ");
        ...
        private void NotifyOnCurrencyChanged(float before, InventoryCurrencyLookup after)
        {
