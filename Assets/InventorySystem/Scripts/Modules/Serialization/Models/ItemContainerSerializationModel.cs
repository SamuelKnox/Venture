        ...
        public virtual InventoryItemSerializationModel[] GetSerializationModels(IEnumerable<InventoryItemBase> items)
        {
            ...
            foreach (var item in items)
            {
                if (item != null)
                {
                    ...
                    {
                        itemModelsList.Add(new InventoryItemSerializationModel((int)item.ID, item.currentStackSize, item.itemCollection.collectionName));
                    }
                    else
                    {
                        itemModelsList.Add(new InventoryItemSerializationModel((int)item.ID, item.currentStackSize, string.Empty));
                    }
                }
                else
                {
                    itemModelsList.Add(new InventoryItemSerializationModel(-1, 0, ""));
                }
            }
