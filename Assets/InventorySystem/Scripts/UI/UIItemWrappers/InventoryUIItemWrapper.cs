...
//                pos.x *= InventoryManager.instance.uiCamera.pixelWidth;
//                pos.y *= InventoryManager.instance.uiCamera.pixelHeight;
        ...
        public virtual void OnEndDrag(PointerEventData eventData)
        {
            ...
            {
                #region Wrapper movement

                // When is large and dragged the end drag event can happen on the wrappers that's being dragged.
                // from     x       x
                // x        to      x
                // x        x       x
        ...
        public virtual void OnPointerUp(PointerEventData eventData)
        {
            ...
            if (InventorySettingsManager.instance.useContextMenu && InventorySettingsManager.instance.triggerContextMenuKeys.AllPressed(eventData, InventoryActionInput.EventType.OnPointerUp))
            {
                if (item != null)
                {
                    TriggerContextMenu();
        ...
        public override void TriggerDrop(bool useRaycast = true)
        {
            ...
            Assert.IsNotNull(InventoryPlayerManager.instance.currentPlayer, "Current player is not set, can't drop item.");
            ...
            if (s.showConfirmationDialogWhenDroppingItem && (s.showConfirmationDialogMinRarity != null && s.showConfirmationDialogMinRarity.ID <= item.rarity.ID))
            {
