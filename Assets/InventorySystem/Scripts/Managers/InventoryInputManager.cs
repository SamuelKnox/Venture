...
namespace Devdog.InventorySystem
{
    ...
    public partial class InventoryInputManager : MonoBehaviour
    {
        ...
        protected static List<GameObject> limitInputTo { get; set; }
        private static InventoryInputManager _instance;

        /// <summary>
        /// An overrider which can be used to globally change the input for item triggers.
        /// For example, when using rewired set this interface to handle the input for rewired.
        /// </summary>
        public IObjectTriggerInputOverrider objectTriggererItemInputOverrider;
        ...
        public static InventoryInputManager instance
        {
            get
            {
                ...
                // if (_instance == null)
                // {
                //     throw new ManagerNotFoundException("InventoryInputManager");
                // }
        ...
        static InventoryInputManager()
        {
            ...
            limitInputTo = new List<GameObject>(8);
        ...
        #region Input limiting

        public void LimitInputTo(GameObject obj)
        {
            limitInputTo.Add(obj);
        }

        public void RemoveLimitInputTo(GameObject obj)
        {
            limitInputTo.RemoveAll(o => o.gameObject == obj);
        }

        public GameObject GetLimitedInputObject()
        {
            if (limitInputTo.Count > 0)
            {
                return limitInputTo[limitInputTo.Count - 1];
            }

            return null;
        }

        public bool IsInputLimitedTo(GameObject obj)
        {
            return GetLimitedInputObject() == obj;
        }

        public bool CanReceiveInput(GameObject obj)
        {
            if (obj == null)
                return false;
            ...
            if (GetLimitedInputObject() == null)
                return true;

            // Input is limited to object, object or all children are allowed to deliver input.
            if (obj == GetLimitedInputObject() || obj.transform.IsChildOf(GetLimitedInputObject().transform))
                return true;
        ...
        private static InventoryUIItemWrapperBase GetWrapperFromRaycastResult(List<RaycastResult> results)
        {
            for (int i = 0; i < results.Count; i++)
            {
                ...
                var wrapper = results[i].gameObject.GetComponent<InventoryUIItemWrapperBase>();
                if (wrapper != null)
                {
                    for (int j = 0; j < i; j++)
                    {
                        // Check if all previous items are a child of this wrapper. If not we're hovering over something else.
                        if (results[j].gameObject.transform.IsChildOf(wrapper.transform) == false)
                        {
                            return null;
                        }
                    }
        ...
        private static List<RaycastResult> GetAllHoveringUIObjects(Vector2 pos)
        {
            ...
            //                position = pos
            //            };
            raycastPointerEventData.position = pos;
        ...
        private static ObjectTriggererBase GetHoveringTriggerer(Vector2 pos)
        {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(pos), out hitInfo, Camera.main.farClipPlane))
            {
                ...
                if (triggerer != null && triggerer.enabled)
                {
