...
//        public CharacterUI character;
        ...
        /// </summary>
        private static List<InventoryCollectionLookup<ItemCollectionBase>> _lootToCollections = new List<InventoryCollectionLookup<ItemCollectionBase>>();
        private static List<InventoryCollectionLookup<CharacterUI>> _equipToCollections = new List<InventoryCollectionLookup<CharacterUI>>(4);
...
//            Assert.IsTrue(item.currentStackSize <= item.maxStackSize, "Item currentStackSize is exceeding the max stack size. This is not allowed, split the item into multiple smaller stacks.");
        ...
        public static bool RemoveItemsThenAdd(InventoryItemAmountRow[] itemsToAdd, params InventoryItemAmountRow[] itemsToRemoveFirst)
        {
            RebuildCounter();
        ...
        /// </summary>
        /// <returns></returns>
        public static bool CanRemoveItemsThenAdd(InventoryItemBase[] itemsToAdd, params InventoryItemAmountRow[] itemsToRemoveFirst)
        {
            return CanRemoveItemsThenAdd(InventoryItemUtility.ItemsToRows(itemsToAdd), itemsToRemoveFirst);
        }


        public static bool CanRemoveItemsThenAdd(InventoryItemAmountRow[] itemsToAdd, params InventoryItemAmountRow[] itemsToRemoveFirst)
        {
            RebuildCounter();
            bool removeSucceeded = _counter.TryRemoveItems(itemsToRemoveFirst);
            if (removeSucceeded == false)
