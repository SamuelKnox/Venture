    ...
    public class InventoryProGettingStarted : EditorWindow
    {
        ...
        public static InventoryProGettingStarted window
        {
            get
            {
                if (_window == null)
                {
                    _window = GetWindow<InventoryProGettingStarted>(true, ProductName + " - Getting started", true);
        ...
        private const string ProductsFetchApiUrl = "http://devdog.io/unity/editorproducts.php?product=InventoryPro";
        private const string ProductName = "Inventory Pro";
        private const string MenuItemPath = "Tools/Inventory Pro/Getting started";
        private const string DocumentationUrl = "http://devdog.io/document/inventory-pro/";
        private const string YoutubeUrl = "https://www.youtube.com/watch?v=kWeXmVIgqO4&list=PL_HIoK0xBTK4R3vX9eIT1QUl-fn78eyIM";
        private const string ForumUrl = "http://forum.devdog.io";
        private const string ReviewProductUrl = "https://www.assetstore.unity3d.com/en/content/31226";

        private const string IconRootPath = "Assets/InventorySystem/EditorStyles/";
        ...
        private static List<ProductInfo> _productInfo = new List<ProductInfo>();

        private static string version
        {
            get { return InventorySettingsManager.version; }
        }
        ...
        private void DrawOtherProducts()
        {
            ...
            EditorGUILayout.LabelField("Other great products you should check out.", EditorStyles.boldLabel);
        ...
        private void DrawMailSignupForm()
        {
            ...
            EditorGUILayout.LabelField("Never miss a thing, sign up for the newsletter.", EditorStyles.boldLabel);
...
//            _name = EditorGUILayout.TextField(_name);
            ...
            if (EditorPrefs.HasKey(DevdogNewsletterSignupKey))
            {
                EditorGUILayout.LabelField("Subscribed with " + EditorPrefs.GetString(DevdogNewsletterSignupKey), EditorStyles.wordWrappedLabel);
            }
        ...
        private void DrawBox(int index, int extraHeight, string title, string desc, Texture texture, Action action)
        {
            ...
            rect.y += 20;
            rect.height = 44;
            GUI.Label(rect, desc, EditorStyles.wordWrappedLabel);
