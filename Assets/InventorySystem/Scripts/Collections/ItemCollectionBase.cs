...
namespace Devdog.InventorySystem
{
    ...
    /// </summary>
    [HelpURL("http://devdog.nl/documentation/collections/")]
    public partial class ItemCollectionBase : MonoBehaviour, IEnumerable, IEnumerable<InventoryUIItemWrapperBase>
    {
        ...
        /// </summary>
        public virtual InventoryUIItemWrapperBase[] items
        {
            get
            {
                return _items;
        ...
        public InventoryCurrencyLookupContainer currenciesContainer { get; set; }
        ...
        {
            get { return items.All(o => o.item == null && currenciesContainer.lookups.All(x => x.amount <= 0f)); }
        }
        ...
        private void RegisterCurrencyLookupEvents()
        {
            ...
            {
                var c = currency;
                c.OnCurrencyChanged += COnOnCurrencyChanged;
            }
        }

        private void UnRegisterCurrencyLookupEvents()
        {
            foreach (var currency in currenciesContainer.lookups)
            {
        ...
        private void COnOnCurrencyChanged(float before, InventoryCurrencyLookup after)
        {
            NotifyCurrencyChanged(before, after);
        }
        ...
        protected virtual void FillUI()
        {
            ...
            {
                items = container.GetComponentsInChildren<InventoryUIItemWrapperBase>();
        ...
        private void NotifyRequireLayoutRedraw()
        {
            if(layoutGroup != null)
        ...
        private void NotifyCurrencyChanged(float before, InventoryCurrencyLookup after)
        {
        ...
        {
            Debug.Log("Adding " + amount + " slots to collection " + collectionName, transform);
            uint oldSize = (uint)items.Length;
        ...
        public virtual bool RemoveSlots(uint amount, bool force = false, bool fireEvents = true)
        {
            ...
            Debug.Log("Removing " + amount + " slots from collection " + collectionName, transform);
            uint oldSize = (uint)items.Length;
            uint newSize = oldSize - amount;
        ...
        public bool CanAddItems(InventoryItemAmountRow[] items)
        {
            RebuildCounterIfDirty();
        ...
        public virtual bool AddItem(InventoryItemBase item, ICollection<InventoryItemBase> storedItems = null, bool repaint = true, bool fireEvents = true)
        {
            ...
            using (new CollectionAvoidRebuildLock(this))
            {
                ...
                {
                    Debug.Log("Tried to put item in collection, but setting - canPutItemsInCollection is false.", gameObject);
                    return false;
                }
        ...
        /// <returns></returns>
        public bool RemoveItemsThenAdd(InventoryItemBase[] itemsToAdd, params InventoryItemAmountRow[] itemsToRemoveFirst)
        {
            RebuildCounterIfDirty();
...
//                var toItem = toCollection[toSlot].item;
                    ...
                    (toCollection.useReferences && fromCollection.useReferences))
                {
                    ...
                    if (moved2 == false)
                    {
                        Debug.LogWarning("Couldn't move 2nd object.");
                    }
                }
...
//                    items[i].item = null;
            ...
            UnRegisterCurrencyLookupEvents();
            currenciesContainer = new InventoryCurrencyLookupContainer(true);
        ...
        /// <returns></returns>
        public InventoryItemBase FindByCategory(uint categoryID)
        {
            foreach (var item in items)
            {
                if (item.item != null && item.item.category.ID == categoryID)
                    return item.item;
            }
        ...
        public InventoryItemBase[] FindAllByCategory(uint categoryID)
        {
            var list = new List<InventoryItemBase>(items.Length);
            foreach (var item in items)
            {
        ...
        public bool CanRemoveItems(IList<InventoryItemAmountRow> items)
        {
            RebuildCounterIfDirty();
        ...
        {
            return FindObjectsOfType<ItemCollectionBase>().FirstOrDefault(o => o.collectionName == collectionName);
        }

        public static T FindByName<T>(string collectionName) where T : ItemCollectionBase
        {
            return FindObjectsOfType<T>().FirstOrDefault(o => o.collectionName == collectionName);
        }
