...
namespace CreativeSpore.SmartColliders
{
    ...
    public class SmartRectCollider2D : SmartCollider2D
    {
        ...
        /// <summary>
        /// The skin raycast will start from body side line for 0f and until the opposite body side line for 1f
        /// </summary>
        ...
        /// </summary>
        protected void DoSolveDynamicCollisions()
        {
            ...
            {
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vRightCheckPoints, vDisp, vDir, Vector3.zero, layerCollision));
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vRightCheckPoints, vDisp, vDir, transform.right, OneWayCollisionRight));
            }
            // check for left movement
            //else if (vLocalDisp.x <= -SkinLeftWidth * Mathf.Abs(transform.localScale.x))
            else if (vLocalDisp.x <= -Vector3.kEpsilon)
            {
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vLeftCheckPoints, vDisp, vDir, Vector3.zero, layerCollision));
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vLeftCheckPoints, vDisp, vDir, -transform.right, OneWayCollisionLeft));
            }
            ...
            {
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vTopCheckPoints, vDisp, vDir, Vector3.zero, layerCollision));
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vTopCheckPoints, vDisp, vDir, transform.up, OneWayCollisionUp));
            }
            ...
            {
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vBottomCheckPoints, vDisp, vDir, Vector3.zero, layerCollision));
                collClosestDist = Mathf.Min(collClosestDist, _DoSolveDynamicCollisionSide(vBottomCheckPoints, vDisp, vDir, -transform.up, OneWayCollisionDown));
            }
            ...
            if (collClosestDist < float.MaxValue)
            {
                ...
                transform.position = m_prevPos + (collClosestDist - k_SkinMinWidth) * vDir; //NOTE: subtracting k_SkinMinWidth avoid precision errors
            }
        }

        protected float _DoSolveDynamicCollisionSide(IList<Vector3> vCheckPoints, Vector3 vDisp, Vector3 vDir, Vector3 vOneWayDir, LayerMask layerMask)
        {
            float collClosestDist = float.MaxValue;
            ...
            for (int i = 0; i < vCheckPoints.Count; ++i)
            {
                ...
                {
                    RaycastHit2D hitInfo2D = _GetClosestHitInfo2D(ray.origin, ray.direction, dist, layerMask);
        ...
        protected Vector3 _DoSolveStaticCollisionSide(IList<Vector3> vCheckPoints, IList<bool> vRayContacts, Vector3 vSkin, Vector3 vOffset, LayerMask layerMask, LayerMask movingPlatformLayerMask)
        {
            ...
            for (int i = 0; i < vCheckPoints.Count; ++i)
            {
                ...
                if (EnableCollision2D)
                {
                    RaycastHit2D hitInfo2D = _GetClosestHitInfo2D(ray.origin, ray.direction, skinMagnitude + fOffsetDist, layerMask);
                    if (hitInfo2D.collider != null && hitInfo2D.collider.gameObject != gameObject)
                    {
                        if (hitInfo2D.collider.isTrigger)
                        {
