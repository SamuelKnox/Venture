...
namespace CreativeSpore.SmartColliders
{
    ...
    public class PlatformCharacterPhysics
    {
        ...
        {
            float sqrt = Mathf.Sqrt(m_vel.y * m_vel.y - 2 * m_gravity.y * (m_pos.y - posY));
            float time = (-m_vel.y - sqrt) / m_gravity.y;
            return time;
        }
        ...
        public float SolveMaxJumpHeight( float jumpSpeed, float jumpAcc = 0f, float jumpAccTime = 0f )
        {
            ...
            else
            {
                ...
                time = -vspeed / m_gravity.y;
                height += vspeed * time + .5f * m_gravity.y * time * time;
                return height;
            }
        }
        ...
        {
            return Mathf.Sqrt( -2f * m_gravity.y * height );
        }
        ...
        public float SolveJumpAccToReachHeight( float height, float jumpSpeed, float jumpAccTime )
        {
            ...
            float v0 = jumpSpeed;            
            float a = -t0 * t0;
            float b = -2f * v0 * t0 + m_gravity.y * t0 * t0;
            float c = 2 * m_gravity.y * (v0 * t0 - height) - v0 * v0;
            ...
            float t1 = -(v0 + A * t0) / m_gravity.y;
            if (t1 <= 0f) // jumpAccTime >= timeToReachHeight
            {
                A = -(v0 * v0) / (2f * height);
            }
            return A - m_gravity.y;            
        }
