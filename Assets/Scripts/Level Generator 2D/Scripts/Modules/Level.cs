namespace LevelGenerator2D
{
    ...
    public class Level : MonoBehaviour
    {
        ...
        public Room[] GetRoomsVisibleByCamera(Camera camera = null)
        {
            ...
            var cameraPoint = camera.transform.position / GridSize;
            var cameraExtents = camera.GetViewport().size / GridSize / 2.0f;
            int minX = Mathf.RoundToInt(cameraPoint.x - cameraExtents.x);
            int maxX = Mathf.RoundToInt(cameraPoint.x + cameraExtents.x);
            int minY = Mathf.RoundToInt(cameraPoint.y - cameraExtents.y);
            int maxY = Mathf.RoundToInt(cameraPoint.y + cameraExtents.y);
        ...
        private void RemoveOutOfScopeRooms()
        {
            ...
            var cameraSize = Camera.main.GetViewport().size;
            var cameraDimensions = new Point2(Mathf.CeilToInt(cameraSize.x / GridSize), Mathf.CeilToInt(cameraSize.y / GridSize));
            int minX = cameraPoint.x - cameraDimensions.x - Room.MaxWidth;
            int maxX = cameraPoint.x + cameraDimensions.x + Room.MaxWidth;
            int minY = cameraPoint.y - cameraDimensions.y - Room.MaxHeight;
            int maxY = cameraPoint.y + cameraDimensions.y + Room.MaxHeight;
