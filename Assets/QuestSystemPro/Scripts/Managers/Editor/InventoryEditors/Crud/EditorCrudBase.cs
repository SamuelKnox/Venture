...
namespace Devdog.QuestSystemPro.Editors
{
    public abstract partial class EditorCrudBase<T> : IEditorCrud where T : class
    {
        ...
        public EditorCrudBase(string singleName, string pluralName, EditorWindow window)
        {
...
//            sidebarRowElementOffset.x += 5; // 110 - 15 extra for offset
            ...
            if (canDeleteItems)
            {
                ...
                {
                    if (UnityEditor.EditorUtility.DisplayDialog("Are you sure?", "Do you want to delete " + singleName + " " + item.ToString(), "Yes", "NO!"))
                    {
                        RemoveItem(i);
                    }
                }
