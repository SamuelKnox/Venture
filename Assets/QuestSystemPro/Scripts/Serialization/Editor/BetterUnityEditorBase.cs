...
//        {
//            // Sometimes happens after compile for some reason.
//            if (target == null)
//            {
//                return;
//            }
...
//        }
        ...
        private List<DrawerBase> GetMainDrawers()
        {
            ...
            ReflectionUtility.GetAllSerializableFieldsInherited(target.GetType(), fieldInfos);
            var drawers = new List<DrawerBase>(fieldInfos.Count);

            foreach (var field in fieldInfos)
            {
                var d = ReflectionDrawerUtility.BuildEditorHierarchy(field, target);
                if (d != null)
                {
                    drawers.Add(d);
                }
                ...
                {
                    QuestLogger.LogWarning("Couldn't create drawer for " + field.Name);
                }
            }
