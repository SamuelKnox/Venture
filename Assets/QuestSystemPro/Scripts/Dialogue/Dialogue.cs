...
//    [SerializationModel(typeof(DialogueSerializationModel))]
    public class Dialogue : BetterScriptableObject
    {
        ...
        public NodeBase[] nodes
        {
            ...
            set
            {
                ...
                {
                    QuestLogger.LogWarning("Setting Dialogue.nodes, but no EntryNode was found in values - Added entry node with index 0.");
        ...
        {
//            QuestLogger.LogVerbose("New dialogue object created");
        }
...
//            }
            ...
            if (nodes.Length <= 1)
            {
                QuestLogger.Log("Can't start a dialogue with with 0 or 1 nodes (start node). Add some nodes first.");
                return false;
            }
        ...
        public bool StartDialogue(IDialogueOwner dialogueOwner, ILocalIdentifier localIdentifier, uint nodeStartIndex = 0)
        {
            ...
            MoveToNextNode(localIdentifier);

            QuestLogger.LogVerbose("Started dialogue");
        ...
        public bool Stop(ILocalIdentifier localIdentifier, bool force = false)
        {
            ...
            QuestLogger.LogVerbose("Stopped dialogue - Resetting current node index to 0 (entry node)");
        ...
        {
            MoveToNextNode(QuestManager.instance.localIdentifier);
        }

        public void MoveToNextNode(ILocalIdentifier localIdentifier)
        {
            var n = currentNode.GetNextNode();
            if (n != null)
            {
                MoveToNextNode(localIdentifier, n);
            }
            else
            {
        ...
        {
            MoveToNextNode(QuestManager.instance.localIdentifier, node);
        }

        public void MoveToNextNode(ILocalIdentifier localIdentifier, NodeBase node)
        {
            currentNode.OnExit();
