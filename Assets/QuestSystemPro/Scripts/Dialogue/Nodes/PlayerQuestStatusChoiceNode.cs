    ...
    public class PlayerQuestStatusChoiceNode : NodeBase, IPlayerDecisionNode
    {
        ...
        public PlayerDecisionsQuestStatus[] playerDecisionsByStatus = new PlayerDecisionsQuestStatus[]
        {
            new PlayerDecisionsQuestStatus()
            {
                ...
                {
                    new PlayerQuestStatusDecision() { option = "Accept", action = QuestStatusAction.Activate }, 
                    new PlayerQuestStatusDecision() { option = "Decline", action = QuestStatusAction.Cancel }, 
                }
            },
            new PlayerDecisionsQuestStatus()
            {
                ...
                playerDecisions = new PlayerDecision[]
                {
                    new PlayerQuestStatusDecision() { option = "Cancel", action = QuestStatusAction.Cancel }, 
                }
            },
            new PlayerDecisionsQuestStatus()
            {
        ...
        public PlayerDecisionsQuestStatus completablePlayerDecision = new PlayerDecisionsQuestStatus()
        {
            ...
            playerDecisions = new PlayerDecision[]
            {
                new PlayerQuestStatusDecision() { option = "Complete", action = QuestStatusAction.Complete },
                new PlayerQuestStatusDecision() { option = "Cancel", action = QuestStatusAction.Cancel },
            }
        };
        ...
        public PlayerDecision[] playerDecisions
        {
            get
            {
                ...
                if (v == null)
                {
                    QuestLogger.LogVerbose("No player decisions found for quest status: " + quest.val.status);
