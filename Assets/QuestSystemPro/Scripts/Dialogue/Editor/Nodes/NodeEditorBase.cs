...
namespace Devdog.QuestSystemPro.Dialogue.Editors
{
    public abstract class NodeEditorBase
    {
        ...
        protected DrawerBase[] drawers = new DrawerBase[0];
        ...
        public virtual void Init(NodeBase node, DialogueEditorWindow editor)
        {
            ...
            _childFields.Clear();
            ReflectionUtility.GetAllSerializableFieldsInherited(node.GetType(), _childFields);

            var d = new List<DrawerBase>();
            foreach (var fieldInfo in _childFields)
            {
                d.Add(ReflectionDrawerUtility.BuildEditorHierarchy(fieldInfo, node));
            }

            this.drawers = d.ToArray();
        }
...
//        public virtual Vector2 CalcNodeSize()
//        {
//            float height = drawers.Sum(o => o.GetHeight()) + ReflectionDrawerStyles.singleLineHeight + NodeInnerPadding * 2;
