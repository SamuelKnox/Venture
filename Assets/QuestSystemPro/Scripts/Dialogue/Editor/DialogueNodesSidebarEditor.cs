...
using UnityEngine;
using UnityEditor;
using EditorStyles = Devdog.QuestSystemPro.Editors.EditorStyles;
                ...
                if (expandedKeys.Contains(kvp.Key) || isSearching)
                {
                    foreach (var type in lookups[kvp.Key])
                    {
                        ...
                        if (GUILayout.Button(type.Name))
                        {
                            ...
                            if (node != null)
                            {
                                QuestLogger.LogVerbose("Add new node of type " + type.Name + " with index: " + node.index);
                            }
                            else
                            {
                                QuestLogger.Log("Couldn't add node, no dialogue selected.");
                            }
                        }
