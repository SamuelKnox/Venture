    ...
    public class QuestSystemInventoryProBridgeManager : MonoBehaviour
    {
        ...
        protected virtual void TaskOnStatusChanged(TaskStatus before, TaskStatus after, Task self)
        {
            if (after == TaskStatus.Active)
            {
                ...
                self.SetProgress(InventoryManager.GetItemCount(iTask.item.val.ID, false));
            }
        }
        ...
        private void InstanceOnPlayerChanged(InventoryPlayer oldPlayer, InventoryPlayer newPlayer)
        {
            if (oldPlayer != null)
            {
                foreach (var inv in oldPlayer.inventoryCollections)
                {
            ...
            if (newPlayer != null)
            {
                foreach (var inv in newPlayer.inventoryCollections)
                {
                    inv.OnAddedItem += InvOnAddedItem;
                    inv.OnRemovedItem += InvOnRemovedItem;
                }
